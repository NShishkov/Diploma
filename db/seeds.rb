...
Contractor.create!([
{:name => "Алексей", :patronymic => "Сергеевич", :surname => "Федоров", :info => "Частный мастер. Работаю в собственном гараже. Специализация на ремонте Европейских и Японских авто. Реставрация старых автомобилей. Ремонт ДВС и КПП. Полировка и химчистка авто. Индвидуальный подход. Низкие цены.",
:address => "г. Москва, ул. Свободная, д. 32", :phone_number => "+79132315432"}, 
{:name => "Сергей", :patronymic => "Владимирович", :surname => "Андреев", :info => "Частный мастер. Опыт работы более 20 лет. Специализация на марка: ВАЗ, УАЗ, ЗАЗ. Кузовной ремонт, слесарные работы. Повышение мощности автомобиля, замена ДВС, тюнинг любой сложности.",
...
roles = Role.create([{:name => 'client', :info => 'Клиент (заказчик)'},
  ...
  {:name => 'new_user', :info => 'Новый пользователь'}])
...
RoleUser.delete_all
RoleUser.create(:role => r3, :user => u1, :us_role_type => "admin")
RoleUser.create(:role => r1, :user => u2, :us_role => con1)
RoleUser.create(:role => r1, :user => u3, :us_role => con2)
RoleUser.create(:role => r2, :user => u4, :us_role => cl1)
RoleUser.create(:role => r2, :user => u5, :us_role => cl2)

puts "---------------------------"
for x in User.all
	puts "emai: #{x.email}"
end
puts "default password: qwerty"
